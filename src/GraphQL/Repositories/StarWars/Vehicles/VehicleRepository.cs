...
public class VehicleRepository([Service] ISwapiService swapi) : IVehicleRepository
{
    ...
    public async Task<SwapiResponseList<VehicleApiResponse>> GetAllAsync(CancellationToken ctx)
    {
        ...
        var pageResults = new ConcurrentDictionary<int, IEnumerable<VehicleApiResponse>>();
        ...
        // Combine all results
        var pageCompleteResult = new List<VehicleApiResponse>();
    ...
    private async Task<SwapiResponseList<VehicleApiResponse>> GetPageAsync(
        int page,
        CancellationToken ctx)
    {
        var uri = $"{this.baseUri}?page={page}";

        var response = await swapi.GetAsync<SwapiResponseList<VehicleApiResponse>>(uri, ctx)
            ?? new SwapiResponseList<VehicleApiResponse>();
