...
public class StarshipRepository([Service] ISwapiService swapi) : IStarshipRepository
{
    ...
    public async Task<SwapiResponseList<StarshipApiResponse>> GetAllAsync(CancellationToken ctx)
    {
        ...
        var pageResults = new ConcurrentDictionary<int, IEnumerable<StarshipApiResponse>>();
        ...
        // Combine all results
        var pageCompleteResult = new List<StarshipApiResponse>();
    ...
    private async Task<SwapiResponseList<StarshipApiResponse>> GetPageAsync(
        int page,
        CancellationToken ctx)
    {
        var uri = $"{this.baseUri}?page={page}";

        var response = await swapi.GetAsync<SwapiResponseList<StarshipApiResponse>>(uri, ctx)
            ?? new SwapiResponseList<StarshipApiResponse>();
