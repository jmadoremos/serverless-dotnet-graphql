...
public class FilmRepository([Service] ISwapiService swapi) : IFilmRepository
{
    ...
    public async Task<SwapiResponseList<FilmApiResponse>> GetAllAsync(CancellationToken ctx)
    {
        ...
        var pageResults = new ConcurrentDictionary<int, IEnumerable<FilmApiResponse>>();
        ...
        // Combine all results
        var pageCompleteResult = new List<FilmApiResponse>();
    ...
    private async Task<SwapiResponseList<FilmApiResponse>> GetPageAsync(
        int page,
        CancellationToken ctx)
    {
        var uri = $"{this.baseUri}?page={page}";

        var response = await swapi.GetAsync<SwapiResponseList<FilmApiResponse>>(uri, ctx)
            ?? new SwapiResponseList<FilmApiResponse>();
